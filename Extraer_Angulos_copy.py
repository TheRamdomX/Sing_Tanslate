    ...
    while True:
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                # Función para calcular ángulo respecto al centro (wrist) y eje Y
                def calculate_center_angle(center, point):
                    vector = point - center
                    angle = degrees(atan2(vector[0], vector[1]))  # Medir respecto al eje Y
                    angle = abs(angle)  # Hacerlo positivo
                    if angle > 180:
                        angle = 360 - angle
